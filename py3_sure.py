	...
	if dpi_status == 'enable' and server_type == 'on-prem':
		...
		if cluster_size == 1:
			if memory_size < 32:
				...
				check_result = 'Failed'
				check_analysis = '''Current memory size is {} GB. Because of current xEdge device count, the memory size is insufficient to perform upgrade.\n
				...
				check_result = 'Failed'
				check_analysis = 'Current memory size is {} GB. But xEdge device count is more than 1500, it exceeds supported scenarios.'.format(memory_size)
			...
			if vedge_count <= 2000 and memory_size < 64:
				...
				check_result = 'Failed'
				check_analysis = 'Current memory size is {} GB. But xEdge device count is more than 5000, it exceeds supported scenarios.'.format(memory_size)
...
def criticalChecktwentythree(cedge_validvsmarts_info, controllers_info, cedge_ip):
	...
	if cedge_ip == None:
		...
		check_action = 'Since no cEdges are connected, check is not required.'
	else:   
...
if __name__ == "__main__":
	...
	try:
		...
		if check_result_one == 'Failed' and check_result_two == 'Failed':
			...
			check_error_logger(log_file_logger, check_result, check_analysis_two, check_count_zfill)
	...
	try:
		if vedge_info:
			...
			cedge_validvsmarts_info = json.loads(getRequestpy3(version_tuple,vmanage_lo_ip, jsessionid, 'device/control/validvsmarts?deviceId={}'.format(cedge_ip), args.vmanage_port, tokenid))
		else:
			cedge_ip = None
